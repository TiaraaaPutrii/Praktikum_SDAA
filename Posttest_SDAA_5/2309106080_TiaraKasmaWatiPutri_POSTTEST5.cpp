...
void tampilkanPakaian() {
    ...
    vector<Pakaian*> pakaianList;

    while (!tempStack.empty()) {
        pakaianList.push_back(tempStack.top());
        tempStack.pop();
    }

    int pilihan;
    tampilkanHeader("Tampilkan Pakaian");
    cout << "1. Tampilkan secara Merge Sort Descending\n";
    cout << "2. Tampilkan secara Quick Sort Ascending\n";
    cout << "3. Tampilkan secara Shell Sort\n";
    cout << "Masukkan pilihan: ";
    cin >> pilihan;
    ...
    switch (pilihan) {
        case 1:
            ...
            sort(pakaianList.begin(), pakaianList.end(), [](Pakaian* a, Pakaian* b) {
                return a->stok > b->stok; // Descending
            });
            break;
        case 2:
            // Quick Sort Ascending
            sort(pakaianList.begin(), pakaianList.end(), [](Pakaian* a, Pakaian* b) {
                return a->stok < b->stok; // Ascending
            });
            break;
        case 3:
            // Shell Sort
            for (int gap = pakaianList.size() / 2; gap > 0; gap /= 2) {
                for (int i = gap; i < pakaianList.size(); i++) {
                    Pakaian* temp = pakaianList[i];
                    int j;
                    for (j = i; j >= gap && pakaianList[j - gap]->stok > temp->stok; j -= gap) {
                        pakaianList[j] = pakaianList[j - gap];
                    }
                    pakaianList[j] = temp;
                }
            }
    ...
    int count = 1;
    for (auto pakaian : pakaianList) {
        cout << "\nPakaian " << count << ":\n";
        cout << "ID            : " << pakaian->id << "\n";
        cout << "Nama Pakaian  : " << pakaian->nama_pakaian << "\n";
        cout << "Stok          : " << pakaian->stok << "\n";
        cout << "Ukuran        : " << pakaian->ukuran << "\n";
        cout << "Kategori      : " << pakaian->kategori_pakaian << "\n";
        count++;
    }
    cout << "====================================\n";
}
